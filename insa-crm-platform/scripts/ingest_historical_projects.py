...
class ProjectIngester:
    ...
    def ingest_project(self, project_folder: Path) -> bool:
        ...
        try:
            logger.info("=" * 80)
            logger.info(f"Ingesting: {project_folder.name}")
            logger.info("=" * 80)
            ...
            logger.info(f"Customer: {metadata.get('customer_name', 'N/A')}")
            logger.info(f"Industry: {metadata.get('industry', 'N/A')}")
            logger.info(f"Date: {metadata.get('completion_date', 'N/A')}")

            # Load requirements
            requirements = self._load_requirements(project_folder)
            if requirements:
                logger.info(f"Requirements loaded ({len(requirements)} chars)")

            # Load quote
            quote_data = self._load_quote(project_folder)
            if quote_data:
                logger.info(f"Quote loaded (${quote_data.get('total_price', 0):,.2f})")

            # Load BOM
            bom = self._load_bom(project_folder)
            if bom:
                logger.info(f"BOM loaded ({len(bom)} parts)")

            # Load outcome
            outcome = self._load_outcome(project_folder)
            if outcome:
                status = outcome.get('status', 'unknown')
                logger.info(f"Outcome: {status}")
            ...
            logger.info(f"Successfully ingested: {project_id}")
            self.stats['successful'] += 1
            return True

        except Exception as e:
            logger.error(f"Failed to ingest {project_folder.name}: {str(e)}")
            self.stats['failed'] += 1
    ...
    def ingest_all(self, projects_dir: Path) -> Dict[str, Any]:
        ...
        if not project_folders:
            logger.warning(f"No project folders found in {projects_dir}")
            return self.stats

        logger.info("=" * 80)
        logger.info("INSA Historical Project Ingestion")
        logger.info("=" * 80)
        ...
        if self.stats['errors']:
            ...
            for error in self.stats['errors']:
                logger.error(f"  - {error['project']}: {error['error']}")
        ...
        logger.info("=" * 80)
        logger.info(f"Total Projects Indexed: {rag_stats.get('total_projects', 0)}")
        logger.info(f"Total Documents: {rag_stats.get('total_documents', 0)}")
...
def main():
    """Main entry point"""
    if len(sys.argv) < 2:
        logger.error("Usage: python3 ingest_historical_projects.py <projects_directory>")
        logger.info("Example:")
        logger.info("  python3 ingest_historical_projects.py /var/lib/insa-crm/historical_projects/")
        sys.exit(1)
