...
class PlatformHealthMonitor:
    ...
    SERVICES = {
        ...
        'mariadb': {
            ...
            'db_test': 'echo "[REDACTED]***" | sudo -S mysql -u root -e "SELECT 1"',
    ...
    def fix_n8n(self) -> bool:
        ...
        if returncode == 0 and stdout.strip():
            ...
            cmd = f"echo '[REDACTED]***' | sudo -S chown -R 1000:1000 {volume_path}"
            returncode, stdout, stderr = self.run_command(cmd)
    ...
    def fix_nginx(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart nginx.service"
    ...
    def fix_postgresql(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart postgresql@16-main.service"
    ...
    def fix_mariadb(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart mariadb.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=30)
    ...
    def fix_redis(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart redis-server.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=10)
    ...
    def fix_postfix(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart postfix@-.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=10)
    ...
    def fix_suricata(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart suricata.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=30)
    ...
    def fix_wazuh(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart wazuh-manager.service"
    ...
    def fix_fail2ban(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart fail2ban.service"
    ...
    def fix_auditd(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S service auditd restart"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_clamav(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart clamav-daemon.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=20)
    ...
    def fix_integrated_healing(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart integrated-healing-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_host_config(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart host-config-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_defectdojo_compliance(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart defectdojo-compliance-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_security_integration(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart security-integration-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_task_orchestration(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart task-orchestration-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_azure_monitor(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart azure-monitor-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
    ...
    def fix_soc_agent(self) -> bool:
        ...
        cmd = "echo '[REDACTED]***' | sudo -S systemctl restart soc-agent.service"
        returncode, stdout, stderr = self.run_command(cmd, timeout=15)
